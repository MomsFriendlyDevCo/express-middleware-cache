...
describe('Cache invalidation (same process)', ()=> {
	...
	after('server teardown', finish => server.close(finish));
	// }}}
...
describe.only('Cache invalidation (seperate process)', ()=> {
	...
	it('should invalidate the cache', done => {
		exec(`node '${__dirname}/scripts/invalidate.js'`, (code, stdout, stderr) => {
			expect(code).to.be.equal(null);
			mlog.log('Subprocess:', stdout);
			done();
		});
	});
